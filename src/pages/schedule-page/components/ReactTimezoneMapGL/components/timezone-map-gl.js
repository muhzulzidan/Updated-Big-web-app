...
const TimezoneMapGL = (props) => {
  ...
  const { defaultMapStyle, defaultViewport, onLoad } = props;
  ...
  const handleHover = (event) => {
    ...
    if (hoveredFeature) {
      ...
      newMapStyle["layers"][FIND_NE_FILL_LAYER]["paint"][
        "fill-opacity"
      ][1][1][2] = neTimeZoneFeature.properties.objectid;
    }
    setState((prev) =>
      Object.assign({}, prev, {
        lngLat,
  ...
  const renderNeTooltip = () => {
    ...
    var dt = DateTime.local().setZone(
      timezoneParser(neTimeZoneFeature.properties.time_zone),
      {
        keepLocalTime: false,
      }
    );
    ...
    if (timezone && Info.isValidIANAZone(timezone)) {
      ...
      layers.push(
        new GeoJsonLayer({
          ...
          getFillColor: [51, 72, 74, 150], //red country changed
          getRadius: 100,
          getLineWidth: 1,
          getElevation: 30,
        })
    ...
    if (hoveredFeature && lngLat) {
      ...
      layers.push(
        new GeoJsonLayer({
          ...
          lineWidthScale: 20,
          lineWidthMinPixels: 2,
          getFillColor: [51, 72, 74, 120], //hovered country color
          getRadius: 100,
          getLineWidth: 1,
          getElevation: 30,
        })
...
export default compose(
  ...
  withProps(({ source, defaultMapStyle, onLoad }) => {
