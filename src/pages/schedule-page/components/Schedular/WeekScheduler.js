...
import {
  actionStatus,
  useEditSettingsClockFormat,
  useEditSettingsStartOfWeek,
  useEditSettingsTimeInterval,
} from "../../endpoints";
...
const Tick = styled.div`
  height: 100%;
  border-left: ${(props) =>
    props.index % props.cellsNumberInInterval === 0
      ? "2px solid rgba(2,26,83, 0.2)"
      : "none"};
  width: ${(props) =>
    props.showLines
      ? props.cellWidth / props.cellsNumberInInterval
      : props.cellWidth}px;
`;
...
const Cell = styled.div`
  border-left: ${(props) =>
      props.index % props.cellsNumberInInterval === 0 ? 2 : 0.5}px
    solid #021a53;
...
const WeekScheduler = ({
  ...
  const schedulerHeight =
    timesHeight + tickHeight + 7 * (cellHeight + spaceBetweenTimelines) + 10;
  ...
  const {
    apiState: editSettingsClockFormatState,
    setApi: setEditSettingsClockFormatApi,
    resetApi: resetEditSettingsClockFormatApi,
  } = useEditSettingsClockFormat({
    ownerID,
    settings: { owner_id: ownerID, clock_format: hourFormat },
  });
  const {
    apiState: editSettingsStartOfWeekState,
    setApi: setEditSettingsStartOfWeekApi,
    resetApi: resetEditSettingsStartOfWeekApi,
  } = useEditSettingsStartOfWeek({
    ownerID,
    settings: { owner_id: ownerID, start_of_week: weekStart },
  });
  const {
    apiState: editSettingsTimeIntervalState,
    setApi: setEditSettingsTimeIntervalApi,
    resetApi: resetEditSettingsTimeIntervalApi,
  } = useEditSettingsTimeInterval({
    ownerID,
    settings: { owner_id: ownerID, time_interval: timeInterval },
  });

  useEffect(() => {
    if (editSettingsClockFormatState === actionStatus.succeeded) {
      setUpdateSettingsIsSnackBarShown(true);
      resetEditSettingsClockFormatApi();
    }
    if (editSettingsStartOfWeekState === actionStatus.succeeded) {
      setUpdateSettingsIsSnackBarShown(true);
      resetEditSettingsStartOfWeekApi();
    }
    if (editSettingsTimeIntervalState === actionStatus.succeeded) {
      setUpdateSettingsIsSnackBarShown(true);
      setZoomOption(zoomOptions.findIndex((x) => x.interval === timeInterval));
      resetEditSettingsTimeIntervalApi();
    }
  }, [
    editSettingsClockFormatState,
    editSettingsStartOfWeekState,
    editSettingsTimeIntervalState,
    resetEditSettingsClockFormatApi,
    resetEditSettingsStartOfWeekApi,
    resetEditSettingsTimeIntervalApi,
    setUpdateSettingsIsSnackBarShown,
  ...
  useHandleOnClickOutside({
    ref: pageRef,
    isDisabled: activeButton === "info" || activeButton === "settings",
    onClick: () => setSelectedItems([]),
    disabledRefs: [actionsPanelRef],
  });
    ...
    if (dragging) {
      ...
      if (maxScroll !== currentScrollLeft) {
        if (e.currentTarget.offsetWidth - x < 120) {
          ...
          rightSpeed.current =
            2 + (120 - (e.currentTarget.offsetWidth - x)) / 40;
        } else {
    ...
    if (!zooming.current) {
      ...
      const focusOnPoint = (newTimelineSize) => {
        ...
        let maxScrollLeft =
          ...
          scrollableContainer.current.clientWidth;
        target =
          target > maxScrollLeft ? maxScrollLeft : target < 0 ? 0 : target;
      ...
      if (direction === "in") {
        let zoomOpt =
          zoomOption < zoomOptions.length - 1 ? zoomOption + 1 : zoomOption;

        focusOnPoint(
          (totalMinutes / zoomOptions[zoomOpt].interval) *
            zoomOptions[zoomOpt].cellWidth
        );
  ...
  useEffect(() => {
    if (activeButton === "info") {
      ...
      setJSON(generateJSON(finalSchedule));
    }
  }, [
    activeButton,
    scheduleId,
  ...
  return (
    <div style={{ marginTop: "-10px" }}>
      <div className="d-flex" style={{ maxWidth: "100%", overflow: "hidden" }}>
        <div className="flex-grow-1 arber" style={{ width: 0 }}>
          <Box
            display="flex"
            alignItems="center"
                ...
                <div style={{ marginRight: `-${leftPadding}px` }}>
                  ...
                  {moment.weekdays(true).map((x, i) => (
                    <div
                      ...
                      className="d-flex"
                      style={{
                        marginBottom: spaceBetweenTimelines,
                        height: cellHeight,
                      }}
                    >
                    ...
                    <div className="position-relative">
                      ...
                      >
                        <div className="d-flex align-items-end position-relative">
                          {new Array(
                            totalMinutes / (showLines ? step : interval)
                          )
                            .fill(0)
                            .map((x, i) => (
                              <TickContainer
        ...
        <div
          ...
          style={{
            ...
            marginRight:
              activeButton === "info" || activeButton === "settings"
                ? 0
                : -sidePanelWidth,
          }}
          ...
          {activeButton === "info" ? (
            ...
            activeButton === "settings" && (
              <SettingsPanel
                onClose={() => setActiveButton("")}
                isLoading={
                  editSettingsClockFormatState === actionStatus.fetching ||
                  editSettingsStartOfWeekState === actionStatus.fetching ||
                  editSettingsTimeIntervalState === actionStatus.fetching
                }
                hourFormat={hourFormat}
                setHourFormat={(hourFormat) => {
                  setHourFormat(hourFormat);
                  setEditSettingsClockFormatApi();
                }}
                weekStart={weekStart}
                setWeekStart={(weekStart) => {
                  setWeekStart(weekStart);
                  setEditSettingsStartOfWeekApi();
                }}
                timeInterval={interval}
                setTimeInterval={(int) => {
                  setTimeInterval(int);
                  setEditSettingsTimeIntervalApi();
                }}
              ></SettingsPanel>
            )
          )}
